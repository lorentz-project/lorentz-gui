; Lorentz Lithium Luna GUI Wallet Installer for Windows
; Copyright (c) 2014-2018, The Lorentz Project
; See LICENSE

[Setup]
AppName=Lorentz GUI Wallet
; For InnoSetup this is the property that uniquely identifies the application as such
; Thus it's important to keep this stable over releases
; With a different "AppName" InnoSetup would treat a mere update as a completely new application and thus mess up

AppVersion=0.12.0.0
DefaultDirName={pf}\Lorentz GUI Wallet
DefaultGroupName=Lorentz GUI Wallet
UninstallDisplayIcon={app}\lorentz-wallet-gui.exe
...
; As of version 0.12.0.0, the Lorentz .exe files do not carry version info anyway in their .exe headers.
...
; applications somehow. Luckily this is no issue here because ALL files are "private" to Lorentz.

Source: "ReadMe.htm"; DestDir: "{app}"; Flags: ignoreversion
Source: "FinishImage.bmp"; Flags: dontcopy

; Lorentz GUI wallet
Source: "bin\lorentz-wallet-gui.exe"; DestDir: "{app}"; Flags: ignoreversion

; Lorentz GUI wallet log file
...
Source: "lorentz-wallet-gui.log"; DestDir: "{app}"; Flags: onlyifdoesntexist; Permissions: users-modify

; Lorentz CLI wallet
Source: "bin\lorentz-wallet-cli.exe"; DestDir: "{app}"; Flags: ignoreversion

; Lorentz wallet RPC interface implementation
Source: "bin\lorentz-wallet-rpc.exe"; DestDir: "{app}"; Flags: ignoreversion

; Lorentz daemon
Source: "bin\lorentzd.exe"; DestDir: "{app}"; Flags: ignoreversion

; Lorentz daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "lorentz-daemon.bat"; DestDir: "{app}"; Flags: ignoreversion;

; Lorentz blockchain utilities
Source: "bin\lorentz-blockchain-export.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\lorentz-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion

; was present in 0.10.3.1, not present anymore in 0.11.1.0 and after
; Source: "bin\lorentz-utils-deserialize.exe"; DestDir: "{app}"; Flags: ignoreversion
...
; Filename: "{app}\lorentz-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  blockChainDefaultDir := ExpandConstant('{commonappdata}\bitlorentz');
  s := 'The default folder to store the Lorentz blockchain is ' + blockChainDefaultDir;
...
begin
  Result := BlockChainDir('') + '\bitlorentz.log';
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "lorentz-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the Lorentz daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\lorentzd.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\lorentz-daemon.bat'), s, false); 
  end;
end;
...
[Icons]
; Icons in the "Lorentz GUI Wallet" program group
; Windows will almost always display icons in alphabetical order, per level, so specify the text accordingly
Name: "{group}\GUI Wallet"; Filename: "{app}\lorentz-wallet-gui.exe"
...
Name: "{group}\Utilities\Lorentz Daemon"; Filename: "{app}\lorentzd.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\lorentz-wallet-cli.exe"; WorkingDir: "{userdocs}\Lorentz\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: "{userdocs}\Lorentz\wallets"
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: "{app}\lorentz-wallet-gui.log"
Name: "{group}\Utilities\x (Try Daemon, Exit Confirm)"; Filename: "{app}\lorentz-daemon.bat"
Name: "{group}\Utilities\x (Try GUI Wallet Low Graphics Mode)"; Filename: "{app}\start-low-graphics-mode.bat"
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM lorentzd.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{userdesktop}\GUI Wallet"; Filename: "{app}\lorentz-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\lorentz-project"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\lorentz-project\lorentz-core"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\lorentz-project\lorentz-core"; ValueType: string; ValueName: "daemonFlags"; ValueData: {code:DaemonFlags};
