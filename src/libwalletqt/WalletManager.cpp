                                    ...
                                    const QString &language, NetworkType::Type nettype)
{
    ...
    Lorentz::Wallet * w = m_pimpl->createWallet(path.toStdString(), password.toStdString(),
                                                  language.toStdString(), static_cast<Lorentz::NetworkType>(nettype));
...
Wallet *WalletManager::openWallet(const QString &path, const QString &password, NetworkType::Type nettype)
{
    ...
    Lorentz::Wallet * w =  m_pimpl->openWallet(path.toStdString(), password.toStdString(), static_cast<Lorentz::NetworkType>(nettype));
...
Wallet *WalletManager::recoveryWallet(const QString &path, const QString &memo, NetworkType::Type nettype, quint64 restoreHeight)
{
    ...
    Lorentz::Wallet * w = m_pimpl->recoveryWallet(path.toStdString(), memo.toStdString(), static_cast<Lorentz::NetworkType>(nettype), restoreHeight);
    m_currentWallet = new Wallet(w);
    return m_currentWallet;
}
                                            ...
                                            quint64 restoreHeight)
{
    ...
    Lorentz::Wallet * w = m_pimpl->createWalletFromKeys(path.toStdString(), language.toStdString(), static_cast<Lorentz::NetworkType>(nettype), restoreHeight,
...
{
    return Lorentz::Wallet::maximumAllowedAmount();
}
...
{
    return QString::fromStdString(Lorentz::Wallet::displayAmount(amount));
}

quint64 WalletManager::amountFromString(const QString &amount) const
{
    return Lorentz::Wallet::amountFromString(amount.toStdString());
}

quint64 WalletManager::amountFromDouble(double amount) const
{
    return Lorentz::Wallet::amountFromDouble(amount);
}

bool WalletManager::paymentIdValid(const QString &payment_id) const
{
    return Lorentz::Wallet::paymentIdValid(payment_id.toStdString());
}

bool WalletManager::addressValid(const QString &address, NetworkType::Type nettype) const
{
    return Lorentz::Wallet::addressValid(address.toStdString(), static_cast<Lorentz::NetworkType>(nettype));
}

bool WalletManager::keyValid(const QString &key, const QString &address, bool isViewKey,  NetworkType::Type nettype) const
{
    std::string error;
    if(!Lorentz::Wallet::keyValid(key.toStdString(), address.toStdString(), isViewKey, static_cast<Lorentz::NetworkType>(nettype), error)){
...
{
    return QString::fromStdString(Lorentz::Wallet::paymentIdFromAddress(address.toStdString(), static_cast<Lorentz::NetworkType>(nettype)));
}
...
{
    return Lorentz::Utils::isAddressLocal(daemon_address.toStdString());
}
...
{
    Lorentz::WalletManagerFactory::setLogLevel(logLevel);
}

void WalletManager::setLogCategories(const QString &categories)
{
    Lorentz::WalletManagerFactory::setLogCategories(categories.toStdString());
}
...
QString WalletManager::checkUpdates(const QString &software, const QString &subdir) const
{
  ...
  const std::tuple<bool, std::string, std::string, std::string, std::string> result = Lorentz::WalletManager::checkUpdates(software.toStdString(), subdir.toStdString());
  if (!std::get<0>(result))
    return QString("");
...
{
    m_pimpl =  Lorentz::WalletManagerFactory::getWalletManager();
}
