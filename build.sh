...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/lorentz-wallet-gui.app/Contents/MacOS/
elif [ "$platform" == "mingw64" ] || [ "$platform" == "mingw32" ]; then
    LORENTZD_EXEC=lorentzd.exe
fi
...
get_tag
popd
echo "var GUI_LORENTZ_VERSION = \"$TAGNAME\"" >> version.js
...
$QMAKE ../lorentz-wallet-gui.pro "$CONFIG" || exit
$MAKE || exit 

# Copy lorentzd to bin folder
if [ "$platform" != "mingw32" ] && [ "$ANDROID" != true ]; then
cp ../$LORENTZ_DIR/bin/$LORENTZD_EXEC $BIN_PATH
fi
