...
{
    // prepare command line arguments and pass to lorentzd
    ...
    qDebug() << "starting lorentzd " + m_lorentzd;
    ...
    // Start lorentzd
    bool started = m_daemon->startDetached(m_lorentzd, arguments);
    ...
    while(true && !m_app_exit) {
        ...
        if(running(nettype)) {
            ...
            if(counter >= 5) {
                qDebug() << "Killing it! ";
#ifdef Q_OS_WIN
                QProcess::execute("taskkill /F /IM lorentzd.exe");
#else
                QProcess::execute("pkill lorentzd");
#endif
            }
...
{ 
    ...
    qDebug() << status;
    // `./lorentzd status` returns BUSY when syncing.
...
bool DaemonManager::sendCommand(const QString &cmd, NetworkType::Type nettype, QString &message) const
{
    ...
    p.start(m_lorentzd, external_cmd);
    ...
    : QObject(parent)
{
    ...
    // Platform depetent path to lorentzd
#ifdef Q_OS_WIN
    m_lorentzd = QApplication::applicationDirPath() + "/lorentzd.exe";
#elif defined(Q_OS_UNIX)
    m_lorentzd = QApplication::applicationDirPath() + "/lorentzd";
#endif

    if (m_lorentzd.length() == 0) {
