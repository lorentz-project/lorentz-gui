...
class WalletListenerImpl : public  Lorentz::WalletListener
{
public:
...
{
    QFuture<Lorentz::Wallet::ConnectionStatus> future = QtConcurrent::run(m_walletImpl, &Lorentz::Wallet::connected);
    QFutureWatcher<Lorentz::Wallet::ConnectionStatus> *connectionWatcher = new QFutureWatcher<Lorentz::Wallet::ConnectionStatus>();

    connect(connectionWatcher, &QFutureWatcher<Lorentz::Wallet::ConnectionStatus>::finished, [=]() {
        QFuture<Lorentz::Wallet::ConnectionStatus> future = connectionWatcher->future();
                                              ...
                                              PendingTransaction::Priority priority)
{
    ...
    Lorentz::PendingTransaction * ptImpl = m_walletImpl->createTransaction(
                dst_addr.toStdString(), payment_id.toStdString(), amount, mixin_count,
                static_cast<Lorentz::PendingTransaction::Priority>(priority), currentSubaddressAccount(), subaddr_indices);
                                                 ...
                                                 quint32 mixin_count, PendingTransaction::Priority priority)
{
    std::set<uint32_t> subaddr_indices;
    Lorentz::PendingTransaction * ptImpl = m_walletImpl->createTransaction(
                dst_addr.toStdString(), payment_id.toStdString(), Lorentz::optional<uint64_t>(), mixin_count,
                static_cast<Lorentz::PendingTransaction::Priority>(priority), currentSubaddressAccount(), subaddr_indices);
...
{
    Lorentz::PendingTransaction * ptImpl = m_walletImpl->createSweepUnmixableTransaction();
    PendingTransaction * result = new PendingTransaction(ptImpl, this);
    return result;
}
...
UnsignedTransaction * Wallet::loadTxFile(const QString &fileName)
{
    ...
    Lorentz::UnsignedTransaction * ptImpl = m_walletImpl->loadUnsignedTx(fileName.toStdString());
...
{
    return QString::fromStdString(Lorentz::Wallet::genPaymentId());
}
...
{
    return QString::fromStdString(m_walletImpl->getDefaultDataDir()) + "/bitlorentz.log";
}
...
Wallet::Wallet(Lorentz::Wallet *w, QObject *parent)
    : QObject(parent)
    , m_walletImpl(w)
    , m_history(nullptr)
...
Wallet::~Wallet()
{
    ...
    delete m_subaddress;
    m_subaddress = NULL;
    //Lorentz::WalletManagerFactory::getWalletManager()->closeWallet(m_walletImpl);
